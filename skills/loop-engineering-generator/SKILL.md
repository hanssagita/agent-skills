...
description: Designs, scaffolds, audits, and executes autonomous closed-loop workflows with objective evidence gates, maker-checker split, bounded iteration budgets, and zero comprehension debt. Adapts to existing repository standards or conducts a rigorous discovery interview for greenfield projects. Make sure to use this skill whenever the user mentions /loop-engineering, loop engineering, agent loops, feedback loops, iterative development, autonomous coding loops, TDD iteration, eval gates, or wants an agent to work in a loop until tests or verification criteria pass.
version: 1.1.0
license: MIT
author: agent-skills
tags: [loop-engineering, autonomous-agents, feedback-loops, verifier-gates, tdd, systems-architecture, karpathy]
...
- **Models:** Claude, GPT, Gemini, Llama, DeepSeek, or local models.
...
- The contract (`LOOP.md`) and verifiers are plain Markdown, Python, and Shell. No proprietary runtime lock-in.
...
```
   ...
   [PATH A: Existing Repository]                   [PATH B: Greenfield Project]
    ...
    - README, package.json, pom.xml                 - Proactively question language,
    - pyproject.toml, Cargo.toml, go.mod              framework, test suite, and goals
    - AGENTS.md, CLAUDE.md, .cursorrules            - Zero assumptions allowed
    - Test configs & linter rules                   - Establish binary success gates
    - Adapt verifier to native stack                - Define strict diff & iteration budget
```
...
1. **Inspect repository configuration files:**
   ...
   - Agent & workspace instructions: `README.md`, `AGENTS.md`, `CLAUDE.md`, `.cursorrules`, `.agent/`, `rules/`.
   - Quality & testing configs: `pytest.ini`, `jest.config.*`, `vitest.config.*`, `Makefile`, `.eslintrc.*`, `ruff.toml`, `tsconfig.json`.
2. **Extract native workflow commands:**
...
Immediately conduct a discovery interview and grill the user on 5 dimensions:
...
| Agent resets completely across sessions with zero memory | **Environment Layer**| Establish workspace contracts, durable memory promotion, and review boundaries. |
...
2. **Context (Hot & Warm State):** The minimal files, repo rules, and rubrics the agent reads before acting.
...
5. **State Persistence:** Durable record outside LLM memory (`loop_state.json` or git commits).
6. **Stop Conditions:**
...
The tool auto-detects existing repository standards (`package.json`, `pyproject.toml`, `Cargo.toml`, `go.mod`, etc.) and sets the verifier command to the native test runner.

This generates:
- `LOOP.md`: The complete loop contract with boundaries, gates, and protocols.
...
Follow the **Plan → Do → Verify → Decide** protocol:

```
1. PLAN   ──> State the single next surgical action and test hypothesis.
2. DO     ──> Apply minimal code change. Keep diffs under 50 lines.
3. VERIFY ──> Execute the objective verifier command in shell.
4. DECIDE ──> If verifier exits 0: Mark complete, report verified diff, STOP.
...
- **Think Before Coding:** State assumptions explicitly. If requirements are ambiguous, ask before looping.
...
- **Zero Comprehension Debt:** Never let the loop generate code faster than human understanding.
...
3. **Check the Diff Budget:** Is the agent adding unasked features or refactoring adjacent code?
4. **Check the Iteration Cap:** Is there an ironclad circuit breaker at 3–4 iterations?
5. **Classify the Failure:** Output the Layer Diagnosis (Harness vs Loop vs Graph vs Environment) and the exact remediation.
...
> A fast autonomous loop operating on an unconstrained repository will generate comprehension debt faster than any engineer can review. Always enforce diff budgets and bounded iterations.

---

## Bundled Resources

- [architecture.md](file:///Users/hans.soegiarto/Documents/hans/agent-skills/skills/loop-engineering-generator/references/architecture.md) — Comprehensive 4-layer taxonomy, mathematical framing, and system boundary reference.
- [failure-modes.md](file:///Users/hans.soegiarto/Documents/hans/agent-skills/skills/loop-engineering-generator/references/failure-modes.md) — The 6 major loop anti-patterns and prevention strategies.
- [scaffold_loop.py](file:///Users/hans.soegiarto/Documents/hans/agent-skills/skills/loop-engineering-generator/scripts/scaffold_loop.py) — Zero-dependency, standards-aware loop scaffolding utility.
- [LOOP_CONTRACT.template.md](file:///Users/hans.soegiarto/Documents/hans/agent-skills/skills/loop-engineering-generator/assets/LOOP_CONTRACT.template.md) — Standalone production contract template.
